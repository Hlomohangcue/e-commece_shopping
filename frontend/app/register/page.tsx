...
export default function RegisterPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetchJson<{ token: string; user: { id: string; email: string; name: string; role: string } }>('/api/auth/register', {
        method: 'POST',
  ...
  return (
    <main className="min-h-[calc(100vh-88px)] bg-slate-950 px-6 py-12 text-slate-100">
      <div className="mx-auto max-w-xl rounded-[2rem] border border-slate-800 bg-slate-900/90 p-10 shadow-glow">
        ...
        <p className="mt-3 text-slate-400">Create an account to save your cart, access orders, and use AI assistance.</p>
        ...
        <p className="mt-6 text-sm text-slate-500">
          Already have an account? <a href="/login" className="text-slate-100 underline">Sign in</a>
        </p>
      </div>
    </main>
  );
